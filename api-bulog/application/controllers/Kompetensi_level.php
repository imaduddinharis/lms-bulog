...
class Kompetensi_level extends REST_Controller {

    function __construct($config = 'rest') {
        parent::__construct($config);
        $this->load->database();
        date_default_timezone_set('Asia/Jakarta');
        $this->table = "kompetensi_level";
    }

    //Menampilkan data user
    function index_get() {
        $kode_level = $this->get('kode_level');
        if ($kode_level == '') {
	...
	function index_post() {
        $data = array(
                    'kode_level'      => $this->post('kode_level'),
	...
	function index_put() {
        $kode_level = $this->put('kode_level');
        $data = array(
	...
	function index_delete() {
        $kode_level = $this->delete('kode_level');
        $this->db->where('kode_level', $kode_level);
